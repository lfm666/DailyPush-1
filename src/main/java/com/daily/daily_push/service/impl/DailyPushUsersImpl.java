...
import com.daily.daily_push.config.ExpressionConfig;
import com.daily.daily_push.entity.HappyState;
import com.daily.daily_push.service.DailyPushUsersService;
import com.daily.daily_push.service.SendMessageService;
import com.daily.daily_push.utils.CnDateUtil;
import com.daily.daily_push.utils.DateCalculation;
import com.daily.daily_push.utils.OtherApi;
...
public class DailyPushUsersImpl implements DailyPushUsersService {
     ...
     * 农历生日
     */
    @Value("${target.rBirthday}")
    private String rBirthday;

    /**
     * 生日类型
     */
    @Value("${target.birthType}")
    ...
    public void sendTextcardMsg() {
        ...
        if (parameterList.getCountBirthday() == 0) {
            ...
            message.append("\uD83C\uDF82距离你下个生日还有").append(parameterList.getCountBirthday()).append("天噢~");
        }
...
//            message.append("\n\uD83D\uDC91今天是我们恋爱的第").append(parameterList.getCountRomanticDate()).append("天噢~");
        ...
        if (parameterList.getWeatherVo() != null) {
            //设置标题
            title = title + parameterList.getWeatherVo().getDate() + " " + parameterList.getWeatherVo().getWeek();

            //消息内容，微信显示不了太多，不要弄太多了
            message.append("\n\n\uD83C\uDFF0城市：").append(parameterList.getWeatherVo().getArea());
...
//            message.append("\n\uD83C\uDFD6生活指数：").append(parameterList.getWeatherVo().getTips());
        ...
        //心情
        String[] split = HappyState.HAPPY_STATE.split("、");
        String state = split[new Random().nextInt(split.length)];
        message.append("\n\n我希望你今天的心情是：").append(state);

        //毒鸡汤
        message.append("\n\uD83E\uDD58毒鸡汤：").append(OtherApi.getDu());
...
//        sendMessage.sendTextcardMsg(userIds,title,message.toString(),url);
        sendMessage.sendNewsMsg(userIds, title, message.toString(), OtherApi.getImgUrl(), OtherApi.getImgUrl());
    }
    ...
    private ParameterListVo getParameterList() {
        ...
        try {
            if (birthType)
                birth = CnDateUtil.lunarToSolar(rBirthday, false);
            else
                birth = birthday;
        } catch (Exception e) {
            e.printStackTrace();
